 ...
 *
 * Validates:
 *   CreateJobSource enum has exactly three values: app, web, twitter
 *   CreateJob.source column defaults to "app"
 *   CreateJob.metadata column is nullable JSON text
 *   CreateJob.joinUrl is nullable
 *   CreateJob.provisioningInstanceId, conversationId, inboxId are nullable
 *   Migration adds source + metadata columns and adds nullable joinUrl/instance columns
 *   Existing CreateJob rows default to source="app" after migration
 */
describe("Twitter-build schema changes", () => {
  // CreateJobSource enum exists with exactly three values
  ...
  // CreateJob.metadata column is nullable JSON
