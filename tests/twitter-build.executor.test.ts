 ...
 *
 * Covers:
 *   Twitter job transitions: pending → generating → done (no provisioning step)
 *   Twitter job generates template using templateGen with idea text
 *   Twitter job publishes template (status=published, firstPublishedAt set, version=1)
 *   Twitter job composes reply text after template is published
 *   Twitter job stores result with templateId, slug, templateUrl, replyText
 *   Twitter job does NOT call ProvisioningClient
 *   Twitter job transitions to failed when templateGen throws
 *   Twitter job transitions to failed with timeout error after 5 minutes
 *   Concurrent twitter + app/web jobs run independently
 *   Twitter job sets expiresAt on terminal state
 */
...
describe("Twitter Build Executor — Happy Path", () => {
  test("twitter job transitions pending → generating → done (no provisioning)", async () => {
  ...
  test("twitter job calls templateGen with idea text from metadata", async () => {
    installTwitterHappyPathMocks();
    const { executeCreateJob } = await import(
      "../src/api/v2/agent-templates/services/job-executor"
    );
  ...
  test("twitter job publishes template (status=published, firstPublishedAt set, version=1)", async () => {
    installTwitterHappyPathMocks();
    const { executeCreateJob } = await import(
      "../src/api/v2/agent-templates/services/job-executor"
    );
  ...
  test("twitter job composes reply after template is published", async () => {
    installTwitterHappyPathMocks();
    const { executeCreateJob } = await import(
      "../src/api/v2/agent-templates/services/job-executor"
    );
  ...
  test("twitter job stores result with templateId, slug, templateUrl, replyText", async () => {
    installTwitterHappyPathMocks();
    const { executeCreateJob } = await import(
      "../src/api/v2/agent-templates/services/job-executor"
    );
  ...
  test("twitter job does NOT call ProvisioningClient", async () => {
    installTwitterHappyPathMocks();
    const { executeCreateJob } = await import(
      "../src/api/v2/agent-templates/services/job-executor"
    );
  ...
  test("twitter job transitions to failed when templateGen throws", async () => {
    __resetGenerateTemplateForTests(() => {
  ...
  test("twitter job transitions to failed with timeout error", async () => {
  ...
  test("concurrent twitter + app/web jobs run independently", async () => {
  ...
  test("twitter job sets expiresAt on terminal state (done)", async () => {
    installTwitterHappyPathMocks();
    const { executeCreateJob } = await import(
      "../src/api/v2/agent-templates/services/job-executor"
    );
  ...
  test("twitter job sets expiresAt on terminal state (failed)", async () => {
    __resetGenerateTemplateForTests(() => {
