...
describe("template-generator-prompt.txt file fidelity", () => {
  test("VAL-M3-PROMPT-001: file exists at expected path", () => {
    expect(existsSync(PROMPT_PATH)).toBe(true);
  });

  test("VAL-M3-PROMPT-002: byte length is 47193", () => {
    expect(statSync(PROMPT_PATH).size).toBe(47193);
  });

  test("VAL-M3-PROMPT-003: SHA256 matches pool source", () => {
...
describe("system-prompt module loading", () => {
  ...
  test("VAL-M3-PROMPT-004: SYSTEM_PROMPT has no leading/trailing whitespace", () => {
  ...
  test("VAL-M3-PROMPT-004: SYSTEM_PROMPT matches readFileSync(...).trim()", () => {
    const fileContent = readFileSync(PROMPT_PATH, "utf8").trim();
    expect(SYSTEM_PROMPT).toBe(fileContent);
  });

  test("VAL-M3-PROMPT-004: SYSTEM_PROMPT is immutable after import (loaded once)", () => {
  ...
  test("VAL-M3-PROMPT-006: SYSTEM_PROMPT content matches file content exactly", () => {
...
describe("system-prompt module graceful error handling", () => {
  test("VAL-M3-PROMPT-005: module does not crash when readFileSync throws at import", () => {
  ...
  test("VAL-M3-PROMPT-005: SYSTEM_PROMPT type allows null for missing prompt", () => {
