# Convos Backend Documentation

This directory contains project documentation, feature plans, and architecture decision records for the Convos backend (Node.js/TypeScript/Express).
...
Good ADR topics for Convos backend:

- **Privacy Model**: Why the backend stores no XMTP identities, only random UUIDs
- **Authentication**: AppCheck for device registration, ES256 JWT for operations
- **Push Notification Flow**: Integration with XMTP notification server, webhook handling
- **Database Schema**: Why only DeviceRegistration and ClientIdentifier tables
- **Invite System**: Self-contained protobuf invites, no server-side storage
