 ...
 *
 * Validates:
 *   CreateJob model exists with all required columns
 *   CreateJobStatus enum has exactly five values
 *   FK from CreateJob.ownerAccountId to Account.id
 *   Account model has inverse relation createJobs
 *   Migration adds CreateJob table with correct SQL types
 *   CreateJob.id defaults to UUID on insert
 *   CreateJob.status defaults to "pending" on insert
 *   CreateJob.status CHECK constraint rejects invalid values
 *   CreateJob.result and CreateJob.error are nullable
 *   CreateJob.expiresAt is nullable
 */
describe("CreateJob schema", () => {
  // CreateJob Prisma model exists with all required columns
  ...
  // Account model has inverse relation
