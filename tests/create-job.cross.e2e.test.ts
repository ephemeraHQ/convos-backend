...
describe("CreateJob Cross-Area E2E", () => {
  ...
  // Full failure path (templateGen error)
  ...
  // Full failure path (provisioning error)
  ...
  // Long-polling returns immediately when job completes
  ...
  // Long-polling times out and returns current status
  ...
  // Production guard blocks both POST and GET
  ...
  // Job with pdfBase64 input flows to done
  ...
  // Job with imageBase64 input flows to done
  ...
  // Expired done job returns 404 on GET
  ...
  // 5-minute timeout triggers failed state
  ...
  // Large base64 payload completes full flow
  ...
  // Job that fails during provisioning still has persisted template
  ...
  // Done job result contains provisioning instance details
  ...
  // Failed job result includes joinFailureReason
  ...
  // GET polling observes all intermediate states in order
  ...
  // Retry after failed job creates new independent job
  ...
  // Job result includes the generated template's ID
  ...
  // PostHog event fires at correct point in state machine
