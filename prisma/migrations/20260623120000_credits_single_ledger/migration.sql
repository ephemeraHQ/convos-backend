...
--
-- 1. New GrantKind rows for the subscription money-in / money-out on the one
--    wallet. `sub_grant` is the per-period allotment; `sub_forfeit`
...
VALUES
  ...
  ('sub_forfeit', 'Subscription forfeit', 'Bounded clawback of unused subscription credits on expiry/refund/revoke', true, CURRENT_TIMESTAMP, CURRENT_TIMESTAMP)
ON CONFLICT ("id") DO NOTHING;

-- 2. Widen the CreditLedger scope domain to admit the subscription clawback
--    scope. Migration 20260522091106 pinned the CHECK to
--    ('transaction', 'grant', 'daily_refill'); the single-ledger forfeit path
--    writes scope = 'sub_forfeit', which that constraint rejects with
...
ALTER TABLE "CreditLedger"
  ...
  CHECK ("scope" IN ('transaction', 'grant', 'daily_refill', 'sub_forfeit'));
