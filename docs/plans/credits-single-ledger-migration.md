...
2. **Subscriptions are money-in/money-out on that wallet.** Subscribe + each renewal write a real `grant` row (idempotent per `{subscriptionId, periodStart}`); expiry/refund writes a bounded `subscription_forfeit` adjustment that removes only the *unused subscription* portion.
...
**Explicitly NOT doing:** no cutover state machine, no dual-write/verify, no backfill-safety harness, no consume-freeze. Those exist to protect *many concurrent* subscribers mid-migration; with n=1 the blast radius is one row you can inspect by hand.
