 ...
 *
 * Covers:
 *   POST returns 400 when no input field provided
 *   POST returns 400 when joinUrl is missing
 *   POST returns 400 when joinUrl is empty string
 *   POST returns 400 when text exceeds 50,000 chars
 *   POST accepts text at exactly 50,000 chars
 *   POST returns 400 when base64 exceeds 35M chars
 *   POST accepts base64 at exactly 35M chars (boundary test)
 *   POST with multiple input types
 *   Body parser accepts up to 40mb payloads
 *   Body parser rejects payloads exceeding 40mb
 */
...
describe("POST /api/v2/agent-templates/create-job — validation", () => {
  ...
  // POST accepts base64 at exactly 35M chars
