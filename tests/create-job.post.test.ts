 ...
 *
 * Covers:
 *   POST returns 202 { jobId }
 *   POST returns 401 without auth
 *   POST returns 401 with invalid JWT
 *   POST accepts X-Agent-API-Key auth
 *   POST creates a pending job
 *   POST stores input as JSON
 *   POST sets ownerAccountId from auth
 *   POST creates job with null expiresAt
 *   POST creates job with null result and error
 *   POST response contains only jobId
 *   POST ignores unknown extra fields
 *   Multiple POSTs create separate jobs
 *   POST accepts pdfBase64 input
 *   POST accepts imageBase64 input
 */
...
describe("POST /api/v2/agent-templates/create-job", () => {
  ...
  // POST creates job with status=pending
