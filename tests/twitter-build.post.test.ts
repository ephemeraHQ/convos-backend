 ...
 *
 * Covers:
 *   POST source=twitter returns 202 when moderation passes
 *   POST source=twitter returns 422 reason=blocked when moderation blocks
 *   POST source=twitter returns 422 reason=not_agent_request when non-agent
 *   POST source=twitter requires idea in metadata
 *   POST source=twitter validates twitterHandle format
 *   POST source=twitter validates tweetId as numeric
 *   POST source=twitter does NOT require joinUrl
 *   POST source=twitter stores metadata JSON
 *   POST source=twitter requires auth
 *   POST source=twitter returns 403 in production
 *   POST source=twitter limits idea to 4000 chars
 *   POST source=twitter returns 400 for empty idea after stripping mentions
 */
...
describe("POST /api/v2/agent-templates/create-job — twitter source", () => {
  ...
  // POST source=twitter stores metadata as JSON
