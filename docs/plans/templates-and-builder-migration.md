...
Tests: full template state machine, slug validation/immutability/reserved-rejection, duplicate-slug acceptance, slug auto-derivation from `agent_name` when body omits it, hashed-slug multi-owner resolution, server-pins-create-fields, DELETE-rejects-published, PATCH-rejects-draft-transitions, builder SSE keep-alive on staging, **first true end-to-end production-guard verification** (live HTTP request against `/api/v2/agent_templates` returns the expected route handler in non-production envs — e.g. `dev`/`staging`/`local`/unset/unknown — and 404s only in exact `production`).
