...
//   truth for `monthlyGrant` in the iOS `CreditBalance` model and the derived
//   subscriber balance (Subscription row + this value, computed at read time —
//   it does NOT write to the ledger). `tierGrant()` (tier-config.ts) reads it
//   lazily and would 500 on the first subscriber read if unset; validating it
