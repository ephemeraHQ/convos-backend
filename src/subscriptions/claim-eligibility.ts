...
export const evaluateClaimable = async (args: {
  ...
  if (lineage.state === LINEAGE_STATE_TOMBSTONED) {
    // Mirror the claim endpoint's gate: with tombstone restoration disabled
    // the claim would 409, so advertising claimable would direct clients
    // into a doomed flow.
    return isTombstoneClaimEnabled();
  }
